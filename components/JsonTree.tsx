...
const JsonTreeNode: React.FC<JsonTreeProps> = ({ data, level = 0, name, isLast = true }) => {
  ...
  const renderValue = (val: any) => {
    ...
    switch (valType) {
      ...
      case 'boolean':
        return <span className="text-red-400 font-bold">{val ? 'true' : 'false'}</span>;
      case 'null':
  ...
  const renderHeader = () => {
    return (
      <div 
        className={`flex items-start group font-mono text-sm leading-6 ${isExpandable && !isEmpty ? 'cursor-pointer hover:bg-gray-800/50 -ml-5 pl-5 rounded' : ''}`}
        onClick={(e) => {
