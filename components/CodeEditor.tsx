...
const CodeEditor: React.FC<ExtendedEditorProps> = ({ 
  ...
  // Handle Parsing for Tree View (JSON, YAML, XML)
  useEffect(() => {
    if (value) {
      if (language === 'json') {
        try {
  ...
  useEffect(() => {
      // Default to tree view for output if it's JSON/YAML/XML and valid
      if ((language === 'json' || language === 'yaml' || language === 'xml') && readOnly && !parseError && value) {
          setViewMode('tree');
      } else {
          setViewMode('code');
      }
  ...
  const canShowTree = language === 'json' || language === 'yaml' || language === 'xml';
          ...
          {canShowTree && (
            <div className="flex bg-gray-900 rounded-md p-0.5 border border-gray-700">
              <button
                ...
                className={`p-1 rounded ${viewMode === 'code' ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-gray-200'}`}
                title="Code View"
              >
              ...
              <button
                ...
                className={`p-1 rounded ${viewMode === 'tree' ? 'bg-blue-600 text-white' : 'text-gray-400 hover:text-gray-200'}`}
                title="Tree View"
        ...
        {canShowTree && viewMode === 'tree' && !parseError ? (
          ...
          <>
            ...
            <textarea
              ...
              className={`flex-1 w-full h-full p-4 bg-gray-900 text-gray-200 font-mono text-sm border-none resize-none focus:ring-0 focus:outline-none leading-6 custom-scrollbar ${readOnly ? 'cursor-text' : 'cursor-text'}`}
              style={{ 
                lineHeight: '1.5rem',
                whiteSpace: 'pre', 
                overflowWrap: 'normal',
        ...
        {canShowTree && viewMode === 'tree' && parseError && (
          <div className="absolute inset-0 flex items-center justify-center bg-gray-900/90 z-10 text-center p-4">
            <div className="text-red-400">
              ...
              <p className="text-sm text-gray-500 mt-1">Switch to Code view to fix syntax errors.</p>
              <button 
                onClick={() => setViewMode('code')}
